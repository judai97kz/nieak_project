                            ...
                            onPressed: () {
                              ...
                              AlertDialog alert = AlertDialog(
                                ...
                                actions: [
                                  ElevatedButton(
                                      onPressed: () {
                                        if(textaccept.text == user.user.value!.password){
                                          User upuser = User(
                                              username: user.user.value!.username,
                                              password: user.user.value!.password,
                                              name: _name.text==""?user.user.value!.name:_name.text,
                                              phone: _phone.text==""?user.user.value!.phone:_phone.text,
                                              address: _address.text==""?user.user.value!.address:_address.text,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() => Container(
            child: Column(
              children: [
                Text(user.user.value!.password.toString()),
                            ...
                            if(textaccept.text == user.user.value!.password){
                              if(newpass.text.length>8){
                                if(newpass.text==passrepeat.text){
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(content: Text('Mật Khẩu Lặp Lại Khác Nhau')));
                                }
                              }else{
                                ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(content: Text('Mật Khẩu Mới Phải Lớn Hơn 8')));
                              }
                            }else{
                              ScaffoldMessenger.of(context).showSnackBar(
